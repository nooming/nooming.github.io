...
- **`app/pixelflow/`**、**`app/citywalk/`** 与 **`app/parking-pso/`**：作为独立交互应用，业务脚本与样式放在各自目录的 **`assets/`**；与站根 **`assets/`** 仅通过 `../../assets/` 共享 favicon、**`common.css`**（Citywalk）、**`utils.js`**（Citywalk Toast 等），避免把应用逻辑散到站根。
